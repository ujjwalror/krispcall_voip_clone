-- Phase 9.X Migration: Round Robin Preferred Agent Fallback Exclusion
-- Date: 2026-09-18
-- Adds optional p_exclude_user_id parameter to reserve_next_round_robin_agent RPC function
-- to allow Round Robin fallback to explicitly exclude the preferred agent who already failed to answer.

DROP FUNCTION IF EXISTS public.reserve_next_round_robin_agent(UUID, UUID, INT);
DROP FUNCTION IF EXISTS public.reserve_next_round_robin_agent(UUID, UUID, INT, UUID);
...
BEGIN
    ...
    --    - NOT equal to p_exclude_user_id (if provided, excludes preferred agent who failed to answer)
...
REVOKE ALL ON FUNCTION public.reserve_next_round_robin_agent(UUID, UUID, INT, UUID) FROM PUBLIC, authenticated;
GRANT EXECUTE ON FUNCTION public.reserve_next_round_robin_agent(UUID, UUID, INT, UUID) TO service_role;
