...
export default function MessagesPage() {
  ...
  return (
    <div className="h-[calc(100vh-7rem)] flex flex-col md:flex-row gap-4">
      {/* Threads Sidebar */}
      <div className="w-full md:w-80 flex flex-col gap-3 bg-slate-900/60 rounded-2xl border border-slate-800/80 p-4">
        ...
        <div className="flex-1 overflow-y-auto space-y-2">
          {threads.map((thread) => (
            <div
              key={thread.id}
              className="p-3 rounded-xl bg-slate-950/60 hover:bg-slate-800/60 border border-slate-800/80 cursor-pointer transition-all"
            >
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-200">{thread.name}</span>
      ...
      <div className="flex-1 flex flex-col bg-slate-900/60 rounded-2xl border border-slate-800/80 overflow-hidden">
        {/* Chat Header */}
        <div className="p-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/40">
          <div className="flex items-center gap-3">
            <Avatar name="John Doe" size="md" status="online" />
            <div>
              <h3 className="text-xs font-bold text-slate-100">John Doe (Acme Corp)</h3>
              <p className="text-[11px] text-slate-400 font-mono">+1 (555) 014-4321</p>
            </div>
          </div>
        ...
        <div className="flex-1 p-4 overflow-y-auto space-y-3">
          <div className="flex flex-col items-start max-w-sm">
            <div className="p-3 rounded-2xl rounded-tl-xs bg-slate-800 text-xs text-slate-200">
          ...
          <div className="flex flex-col items-end max-w-sm ml-auto">
            ...
            <div className="flex items-center gap-1 mt-1 mr-1 text-[9px] text-slate-400 font-mono">
              <span>10:15 AM</span>
