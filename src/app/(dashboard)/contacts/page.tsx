...
export default function ContactsPage() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 w-full sm:w-auto">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className="flex-1 sm:flex-none whitespace-nowrap font-semibold shadow-md shadow-blue-600/20 justify-center"
            >
      ...
      {blockedAlertMessage && (
        <div className="p-3.5 rounded-xl bg-rose-50 dark:bg-rose-950/80 border border-rose-200 dark:border-rose-700 text-rose-800 dark:text-rose-200 text-xs flex items-center justify-between animate-in fade-in">
          <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="text-rose-600 dark:text-rose-400 hover:text-rose-900 dark:hover:text-rose-200 ml-3"
          >
            ✕
          </button>
        </div>
      ...
      {isLoading ? (
        <Card className="p-12 text-center text-xs text-slate-500">
          <RefreshCw className="w-6 h-6 animate-spin mx-auto mb-3 text-blue-600 dark:text-blue-400" />
        ...
        <Card className="p-12 text-center text-xs text-slate-500 flex flex-col items-center justify-center gap-3">
          <div className="p-4 rounded-2xl bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-500">
            <Users className="w-8 h-8" />
          </div>
          <div>
            <h3 className="text-sm font-bold text-slate-800 dark:text-slate-300">No Contacts Found</h3>
            {searchQuery ? (
              ...
              <div className="space-y-3">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100">{contact.full_name}</h3>
                        {contact.is_blocked && (
                      ...
                      {contact.company ? (
                        ...
                        <p className="text-xs text-slate-500 flex items-center gap-1 mt-0.5">
                          <UserCheck className="w-3 h-3 text-slate-400 dark:text-slate-500 shrink-0" />
                  ...
                  <div className="relative" ref={activeMenuContactId === contact.id ? menuRef : null}>
                    <button
                      ...
                      className="p-1.5 rounded-lg text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                    ...
                    {activeMenuContactId === contact.id && (
                      <div className="absolute right-0 top-8 z-30 w-44 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl p-1.5 text-xs space-y-0.5 animate-in fade-in">
                        <button
                          onClick={() => {
                            setActiveMenuContactId(null);
                            setSelectedDetailsId(contact.id);
                          }}
                          className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-100 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                        >
                        ...
                        {contact.is_blocked ? (
                          <button
                            ...
                            className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-950/60 transition-colors"
                          >
                          ...
                          <button
                            ...
                            className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/60 transition-colors"
                          >
                        ...
                        <button
                          ...
                          className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/60 transition-colors"
                        >
                ...
                <div className="space-y-1.5 pt-2 border-t border-slate-100 dark:border-slate-800 text-xs">
                  <p className="text-slate-900 dark:text-slate-300 font-mono flex items-center gap-2">
                    <PhoneCall className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400 shrink-0" />
                    <span>{contact.phone}</span>
                  </p>
                  {contact.email && (
                    <p className="text-slate-500 dark:text-slate-400 flex items-center gap-2 truncate">
                ...
                {contact.notes && (
                  <p className="text-[11px] text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-950/60 p-2.5 rounded-lg border border-slate-200 dark:border-slate-800/80 line-clamp-2">
                    {contact.notes}
                  </p>
                )}
              </div>
