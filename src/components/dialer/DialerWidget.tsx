    ...
    <div className="w-full max-w-sm mx-auto rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl dark:shadow-2xl p-4 sm:p-5 flex flex-col gap-3.5 transition-colors">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 pb-3 border-b border-slate-100 dark:border-slate-800">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {identity && (
            <span className="text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400 font-mono truncate max-w-[150px]">Agent: {identity}</span>
          )}
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 text-xs px-3 sm:px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800/80">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 text-xs">
        <div className="flex items-center gap-2">
          <Radio className={`w-3.5 h-3.5 ${recordCallPreference ? 'text-rose-500 animate-pulse' : 'text-slate-400 dark:text-slate-500'}`} />
          <span className="text-slate-700 dark:text-slate-300 font-medium text-[11px]">
            Call Recording: <strong className={recordCallPreference ? 'text-rose-600 dark:text-rose-400' : 'text-slate-500 dark:text-slate-400'}>{recordCallPreference ? 'ON' : 'OFF'}</strong>
          </span>
        </div>
        <button
          ...
          className={`px-2.5 py-1 rounded-lg text-[10px] font-bold tracking-wider uppercase transition-colors border ${
            recordCallPreference
              ? 'bg-rose-100 dark:bg-rose-950/80 border-rose-300 dark:border-rose-600/80 text-rose-700 dark:text-rose-300 hover:bg-rose-200 dark:hover:bg-rose-900/80'
              : 'bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-slate-200'
          }`}
        >
      ...
      {callState !== 'idle' && !incomingCaller && (
        ...
        >
          <div className="flex items-center gap-2">
            ...
            {callState === 'connected' && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />}
            {callState === 'permission_denied' && <MicOff className="w-3.5 h-3.5 text-rose-600 dark:text-rose-400" />}
          ...
          {callState === 'connected' && (
            <button
              ...
              className={`p-1.5 rounded-lg border transition-colors ${
                isMuted
                  ? 'bg-rose-100 dark:bg-rose-900/60 border-rose-300 dark:border-rose-500 text-rose-800 dark:text-rose-200'
      ...
      {/* Error Alert Display */}
      {errorMessage && (
        <div className="p-2.5 rounded-xl bg-rose-50 dark:bg-rose-950/60 border border-rose-200 dark:border-rose-800/80 text-[11px] text-rose-800 dark:text-rose-200 flex items-start justify-between">
          <div className="flex items-center gap-1.5">
            <AlertTriangle className="w-3.5 h-3.5 text-rose-600 dark:text-rose-400 shrink-0 mt-0.5" />
            <span>{errorMessage}</span>
          </div>
          <button onClick={clearError} className="text-rose-600 dark:text-rose-400 hover:text-rose-900 dark:hover:text-rose-200 ml-2">
            ✕
          </button>
        </div>
      )}
      ...
      <div className="relative">
        <Input
          ...
          disabled={isCallActive}
          className="text-center text-lg font-mono tracking-wider font-semibold py-3"
        />
      ...
      <div className="grid grid-cols-3 gap-2.5 my-1">
        {keys.map((key) => (
          <button
            ...
            className="flex flex-col items-center justify-center h-14 rounded-xl bg-slate-50 hover:bg-slate-100 dark:bg-slate-950/60 dark:hover:bg-slate-800/80 border border-slate-200 dark:border-slate-800/80 active:bg-blue-50 dark:active:bg-blue-600/20 active:border-blue-300 dark:active:border-blue-500/40 transition-all duration-100 group"
          >
            <span className="text-base font-semibold text-slate-900 dark:text-slate-100 group-hover:scale-105 transition-transform">
              {key.num}
            </span>
      ...
      <div className="flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400 px-1 pt-2 border-t border-slate-100 dark:border-slate-800/80">
        <div className="flex items-center gap-1.5">
