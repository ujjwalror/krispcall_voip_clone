...
(() => {
  ...
  // chrome-extension/src/dialer-window.ts
  var device = null;
  var activeCall = null;
  ...
  var isMuted = false;
  var authToken = null;
  var isDeviceInitializing = false;
  var currentCrmContext = null;
  ...
  function updateStatusBadge(text, color, borderColor) {
  ...
  async function getStoredAuthToken(forceRefresh = false) {
    if (!forceRefresh) {
  ...
  async function fetchBusinessNumbers() {
    const selectElem = document.getElementById("business-number-select");
  ...
  async function executeOutboundCall(destinationPhone) {
    ...
    const fromSelect = document.getElementById("business-number-select");
    const recToggle = document.getElementById("record-toggle");
