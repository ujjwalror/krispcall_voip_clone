...
(() => {
  ...
  var currentCrmContext = {
    recordName: "Zoho CRM Record",
    phoneNumbers: []
  };
  function cleanAndNormalizePhoneText(rawText) {
  ...
  function scanZohoRecordFields() {
    ...
    for (const elem of allElements) {
      ...
      if (text && text.length < 35 && phoneLabelRegex.test(text)) {
        ...
        let valElem = elem.nextElementSibling;
        if (!valElem && elem.parentElement) {
          valElem = elem.parentElement.nextElementSibling;
        }
        if (!valElem) {
          valElem = elem.querySelector('.crm-fld-val, .fieldValue, [class*="val"], [class*="Val"], span, td');
        }
        if (valElem) {
          if (valElem.closest("#voiphub-crm-floating-panel")) continue;
          const clone = valElem.cloneNode(true);
    ...
    attrElems.forEach((elem) => {
      ...
      if (elem.tagName === "INPUT" || elem.tagName === "TEXTAREA" || elem.isContentEditable) return;
      ...
      if (isPhone && !seenValues.has(normalized)) {
        ...
        if (!elem.hasAttribute("data-voiphub-injected")) {
          ...
          injectCrmCallButton(elem, normalized, labelAttr);
        }
      }
    });
  ...
  function injectCrmCallButton(targetElem, phone, label) {
